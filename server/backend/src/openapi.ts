...
const toOperationId = (method: string, routePath: string) => {
  const normalized = routePath.replaceAll(/[{}]/g, "").replaceAll("/", "_").replaceAll(/[^A-Za-z0-9_]/g, "_");
...
const hasUndefined = (type: ts.Type) =>
  type.isUnion() ? type.types.some((item) => (item.flags & ts.TypeFlags.Undefined) !== 0) : (type.flags & ts.TypeFlags.Undefined) !== 0;
...
const typeToSchema = (
  ...
  if ((type.flags & ts.TypeFlags.Any) !== 0 || (type.flags & ts.TypeFlags.Unknown) !== 0 || (type.flags & ts.TypeFlags.Never) !== 0) {
    return {};
  }
  ...
  if (type.isUnion()) {
    ...
    if (isLiteralUnion(withoutNull)) {
      ...
      if (booleans.length === withoutNull.length) {
        const boolValues = booleans
          .map((item) => inferBooleanLiteral(item))
...
const analyzeHandlerWithTypes = (
  ...
  for (const [status, schemas] of responseSchemas.entries()) {
    ...
    responses[status] = {
      description: status.startsWith("2") ? "Successful response" : status.startsWith("3") ? "Redirect response" : "Error response",
      content: {
        "application/json": {
...
const buildTypedOperationMap = () => {
  ...
  const appSource = program
    .getSourceFiles()
    .find((source) => /[/\\]src[/\\]app\.ts$/.test(source.fileName));
