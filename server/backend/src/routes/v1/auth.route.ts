...
const registerSchema = z.object({
  ...
  password: z.string().min(1).max(MAX_PASSWORD_LENGTH).refine((value) => isPasswordComplexEnough(value), {
...
const resetPasswordSchema = z.object({
  ...
  newPassword: z.string().min(1).max(MAX_PASSWORD_LENGTH).refine((value) => isPasswordComplexEnough(value), {
    message: PASSWORD_COMPLEXITY_ERROR_MESSAGE
  })
});
