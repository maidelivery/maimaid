...
export const mergeChartStatsPayloads = (
  ...
  for (const songId of songIds) {
    ...
    const merged = Array.from({ length: maxLength }, (_, index) => {
      ...
      const rightEntry = rightEntryCandidate && rightEntryCandidate.cnt >= secondaryMinCnt
        ? rightEntryCandidate
        : null;
  ...
  return {
    charts: Object.fromEntries(
      Object.entries(mergedCharts).map(([songId, entries]) => [songId, toChartArrayOutput(entries)])
    ),
    diff_data: diffData
  };
};
...
export class ChartFitService {
  ...
  private resolveSongIdFromMapping(
    ...
    const preferred = normalizedType === "dx"
      ? candidates.find((id) => id >= 10000)
      : candidates.find((id) => id > 0 && id < 10000);
