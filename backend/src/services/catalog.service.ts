...
export class CatalogService {
	...
	private lxnsRemoteAliasCache: { sourceUrl: string; expiresAt: number; items: LxnsAliasItem[] } | null = null;
	...
	private async listLxnsAliasesFromRemote(songIdentifier: string | undefined, songIdentifierByNameMap: Map<string, string>) {
		try {
			...
			const targetUrl =
				source?.enabled && source.activeUrl ? source.activeUrl : "https://maimai.lxns.net/api/v0/maimai/alias/list";
			const rawItems = await this.loadRemoteLxnsAliasItems(targetUrl);
			if (rawItems.length === 0) {
				return [];
			}
			const knownSongIdentifiers = new Set(songIdentifierByNameMap.values());
	...
	private async loadRemoteLxnsAliasItems(targetUrl: string): Promise<LxnsAliasItem[]> {
		const now = Date.now();
		if (
			this.lxnsRemoteAliasCache &&
			this.lxnsRemoteAliasCache.sourceUrl === targetUrl &&
			this.lxnsRemoteAliasCache.expiresAt > now
		) {
			return this.lxnsRemoteAliasCache.items;
		}
		...
		const payload = (await response.json()) as unknown;
		const items = this.extractLxnsItems(payload);
		this.lxnsRemoteAliasCache = {
			sourceUrl: targetUrl,
			expiresAt: now + LXNS_REMOTE_ALIAS_CACHE_TTL_MS,
			items,
		};
		return items;
	}
	...
	private async getSongIdentifierByNameMap() {
		const now = Date.now();
		if (this.songIdentifierByNameCache && this.songIdentifierByNameCache.expiresAt > now) {
			return this.songIdentifierByNameCache.value;
		}
		...
		this.songIdentifierByNameCache = {
			expiresAt: now + SONG_IDENTIFIER_BY_NAME_CACHE_TTL_MS,
			value: map,
		};
		return map;
	}
