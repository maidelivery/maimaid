...
import type { Alias, Song } from "@/components/songs/types";
...
export function useDashboardActions(input: UseDashboardActionsInput) {
  ...
  const handleUpdateStaticBundleSchedule = async (input: {
    enabled: boolean;
  ...
  const handleRegisterPasskey = async (): Promise<string | null> => {
    try {
      ...
      const browserResponse = await startRegistration({ optionsJSON: options as Parameters<typeof startRegistration>[0]["optionsJSON"] });
  ...
  const handleRegenerateBackupCodes = async (): Promise<BackupCodeStatus & { codes: string[]; generatedAt: string } | null> => {
    const confirmed = await confirmAction({
