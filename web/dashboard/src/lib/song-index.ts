...
export function buildSongCatalogIndex(input: {
  ...
  const songSnapshots = songs.map((song) => {
    ...
    const fallbackIds = providerSongIdsByName.get(normalizeSearchText(song.title))
...
function buildAliasIndex(input: { songs: Song[]; aliases: Alias[]; songIdItems: SongIdItem[] }): Map<string, string[]> {
  ...
  return new Map(
    Array.from(map.entries()).map(([songIdentifier, aliasSet]) => [
      songIdentifier,
      Array.from(aliasSet.values()),
    ]),
  );
}
