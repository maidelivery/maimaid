                  ...
                  >
                    <div className="flex items-start gap-3">
                      <Avatar className="size-12 rounded-md">
                        <AvatarImage src={resolveSongCoverUrl(row.songIdentifier) ?? undefined} alt={resolveSongTitle(row.songIdentifier)} />
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 break-all text-xs text-muted-foreground">{t("aliasPrefix")}{row.aliasText}</p>
                        <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                          <span className="rounded-md border px-2 py-1">{t("supportCountPrefix", { count: row.supportCount })}</span>
                          <span className="rounded-md border px-2 py-1">{t("opposeCountPrefix", { count: row.opposeCount })}</span>
                        </div>
                        <p className="mt-2 text-xs text-muted-foreground">{t("closeAtPrefix")}{formatDateTime(row.voteCloseAt)}</p>
                      </div>
                    </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-3 sm:flex-row">
                <Avatar className="size-24 rounded-md">
                  ...
                  />
                  <AvatarFallback>{resolveSongTitle(selectedCandidate.songIdentifier).slice(0, 1).toUpperCase()}</AvatarFallback>
                </Avatar>
                <div className="flex flex-1 flex-col gap-2">
                  <p className="text-base font-medium">{resolveSongTitle(selectedCandidate.songIdentifier)}</p>
                  <p className="text-sm text-muted-foreground">{t("aliasPrefix")}{selectedCandidate.aliasText}</p>
                  <div className="flex flex-wrap gap-2">
                    <Badge variant="secondary">{t("statusPrefix")}{selectedAdminCandidate?.status ?? selectedCandidate.status ?? "voting"}</Badge>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <InfoBlock
                  label={t("infoMyVote")}
                  value={selectedCandidate.myVote === 1 ? t("voteSupport") : selectedCandidate.myVote === -1 ? t("voteOppose") : t("voteNone")}
                />
                  ...
                  <FieldGroup>
                    <Field>
                      ...
                      <Input
                        ...
                        onChange={(event) =>
                          onCandidateVoteCloseDraftChange(selectedCandidate.candidateId, event.target.value)
                        }
                      />
                    </Field>
                  </FieldGroup>
