...
function App() {
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      setMyCandidateSongName: state.setMyCandidateSongName,
      myCommunityRows: state.myCommunityRows,
      setMyCommunityRows: state.setMyCommunityRows,
      approvedAliasRows: state.approvedAliasRows,
      ...
      setAdminCandidateStatus: state.setAdminCandidateStatus,
  ...
  const catalogVersionSequence = useMemo(
    () => catalogVersionItems.map((item) => item.version),
    [catalogVersionItems],
  );

  const versionSortOrder = useCallback((version: string) => {
    const exactIndex = catalogVersionSequence.findIndex((item) => item === version);
    if (exactIndex >= 0) {
      return exactIndex;
    }

    const matches = catalogVersionSequence.map((item, index) => ({ item, index })).filter(
      ({ item }) => version.includes(item) || item.includes(version),
    );
    if (matches.length === 0) {
      return 999;
    }
  ...
  const formatVersionDisplay = useCallback((version?: string | null) => {
    if (!version) {
      return "-";
    }
    ...
    const matches = catalogVersionItems.filter(
      (item) => version.includes(item.version) || item.version.includes(version),
    );
    if (matches.length === 0) {
      return version;
    }
  ...
  const allVersions = useMemo(
    () =>
      Array.from(new Set(songCatalog.map((song) => song.version).filter((item): item is string => Boolean(item))))
        .sort((a, b) => {
  ...
  const filteredScores = useMemo(() => {
    ...
    return scores.filter((row) => {
      ...
      return (
        localizedStandardContains(fallbackTitle, searchText) ||
        localizedStandardContains(songIdentifier, searchText)
      );
    });
  ...
  const filteredSongs = useMemo(() => {
    ...
    const filteredSnapshots = songSnapshots
      .filter((snapshot) => {
      ...
      if (songSortOption === "difficulty") {
        if (left.maxDifficulty !== right.maxDifficulty) {
          return songSortAscending
            ...
            : right.maxDifficulty - left.maxDifficulty;
        }
        const fallbackLeft = left.song.sortOrder ?? (songCatalogIndexByIdentifier.get(left.song.songIdentifier) ?? 999999);
        const fallbackRight = right.song.sortOrder ?? (songCatalogIndexByIdentifier.get(right.song.songIdentifier) ?? 999999);
        return fallbackLeft - fallbackRight;
  ...
  const request = useCallback(
    async <T,>(
      ...
      requestJson<T>(BACKEND_URL, path, session, setSession, clearSession, options),
  ...
  const toggleSongFavorite = useCallback((songIdentifier: string) => {
  ...
  }, [activeProfileId, request, session]);

  const loadCommunity = useCallback(async () => {
    const boardPromise = request<{ rows: CommunityCandidate[] }>(
      "v1/community/aliases/voting-board?limit=120&offset=0",
      {
  ...
  const loadMyCommunity = useCallback(async () => {
    ...
    const suffix = matchedSong
      ? `?songIdentifier=${encodeURIComponent(matchedSong.songIdentifier)}&limit=30`
      : "?limit=30";
    const payload = await request<{ rows: MyCommunityCandidate[] }>(
      `v1/community/aliases/my-candidates${suffix}`,
    );
    setMyCommunityRows(payload.rows);
  }, [myCandidateSongName, request, resolveSongByName, session, showToast]);
  ...
  }, [adminCandidateStatus, isAdmin, request]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        await Promise.all([loadProfiles(), loadCommunity(), loadSongCatalog(), loadMfaStatus(), loadPasskeys(), loadBackupCodeStatus()]);
        if (isAdmin) {
      ...
      <div className="dark min-h-screen bg-background text-foreground">
        <main className="mx-auto flex min-h-screen w-full max-w-md flex-col items-center justify-center gap-4 px-4 py-10">
          <UiAlert>
            ...
            <AlertDescription>
              {t("app:continueAppDesc1")}{session.user.email}{t("app:continueAppDesc2")}
            </AlertDescription>
          </UiAlert>
  ...
  const selectedProfileName =
    ...
    enabledProfile?.name?.trim() ||
    t("app:unnamedProfile");
