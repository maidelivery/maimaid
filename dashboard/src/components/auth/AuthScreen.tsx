...
export function AuthScreen(props: AuthScreenProps) {
  ...
  const normalizedBackupCode = mfaBackupCode.trim().toUpperCase().replace(/[^A-Z0-9]/gu, "");
  ...
  return (
    <div className="dark min-h-screen bg-[#2f2f31] text-foreground">
                ...
                {verificationResult ? (
                  ...
                  <>
                    <p className="text-sm text-muted-foreground">
                      {t("auth:verifyCreated1")}{verificationEmail}{t("auth:verifyCreated2")}
                    </p>
                    <p className={cn("text-sm", verificationEmailSent ? "text-emerald-500" : "text-yellow-500")}>
                      {verificationEmailSent
                        ? t("auth:verifySent")
                        : t("auth:verifyFailSend")}
                    </p>
