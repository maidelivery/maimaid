							...
							<div className="flex min-w-0 flex-1 flex-col gap-2">
								...
								{song.searchKeywords ? (
									<p className="text-sm text-muted-foreground">
										{t("detailKeywordPrefix")}
										{song.searchKeywords}
									</p>
								) : null}
								{song.comment ? (
									<p className="text-sm text-muted-foreground">
										{t("detailCommentPrefix")}
										{song.comment}
									</p>
								) : null}
								<div className="flex flex-wrap gap-2">
									<Badge variant="secondary">
										🇯🇵 JP {selectedSongRegionSummary.jp ? t("detailPlayable") : t("detailUnplayable")}
									</Badge>
									<Badge variant="secondary">
										🌏 INTL {selectedSongRegionSummary.intl ? t("detailPlayable") : t("detailUnplayable")}
									</Badge>
									<Badge variant="secondary">
										🇨🇳 CN {selectedSongRegionSummary.cn ? t("detailPlayable") : t("detailUnplayable")}
									</Badge>
								</div>
								<div className="flex flex-wrap gap-2">
										...
										{detailSheetsPagination.pagedItems.map((sheet) => (
											<article key={sheet.id} className="rounded-lg border p-3">
												<div className="flex flex-wrap items-center gap-2">
													...
													<Badge variant="secondary">
														{t("detailColLevel")} {sheet.level}
													</Badge>
													<Badge variant="secondary">
														{t("detailColInternal")} {sheet.internalLevel ?? "-"}
													</Badge>
													{sheet.isSpecial ? <Badge variant="secondary">{t("detailBadgeSpecial")}</Badge> : null}
												</div>
												<p className="mt-2 text-xs text-muted-foreground">
													{t("detailColDesigner")}：{sheet.noteDesigner ?? "-"}
												</p>
										...
										{ratingRowsPagination.pagedItems.map((row) => (
											<article key={`rating-row-mobile-${row.rank}`} className="rounded-lg border p-3">
												...
												<p className="mt-2 text-sm font-medium">
													{t("detailRatingColRating")}：{row.rating}
												</p>
											</article>
										))}
									</div>
