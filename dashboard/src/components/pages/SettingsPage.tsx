...
export function SettingsPage({
  ...
  const passkeyNamedCount = useMemo(
    () => passkeys.filter((item) => Boolean(item.name?.trim())).length,
    [passkeys],
  );
            ...
            <section className="mt-4 rounded-xl border border-border/70 p-4">
              ...
              <div className="mt-4 flex flex-col gap-4">
                <div className="rounded-lg border border-border/60 bg-muted/20 p-3">
                  ...
                  <p className="truncate text-xs text-muted-foreground">{selectedProfile?.id ?? t("pleaseSelectViewProfile")}</p>
                </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_220px]">
                    <div className="space-y-3">
                      ...
                      <Field>
                        ...
                        <Input id="totp-code" value={mfaSetupCode} onChange={(event) => onMfaSetupCodeChange(event.target.value)} />
                      </Field>
                    ...
                    <div className="mx-auto flex w-[220px] flex-col items-center gap-2">
                      <div className="flex h-[220px] w-[220px] items-center justify-center rounded-lg border border-border/60 bg-background p-2">
                        {totpQrDataUrl ? (
                          <img src={totpQrDataUrl} alt="TOTP QR Code" className="h-full w-full rounded-md object-contain" />
                        ) : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("backupCodesStatus", { activeCount: backupCodeStatus.activeCount, latestGeneratedAt: formatDateTime(backupCodeStatus.latestGeneratedAt, t) })}
                    </p>
                  </div>
                  <Button
                    size="sm"
                  ...
                  <div className="mt-3 rounded-lg border border-border/60 bg-muted/20 p-3">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      {backupCodes.map((code) => (
                        <div key={code} className="rounded-md border border-border/60 bg-background px-2 py-1.5 font-mono text-sm">
                          {code}
                        </div>
                      ))}
                    </div>
                    <p className="mt-2 text-xs text-muted-foreground">
                      {t("backupCodeUsage1")}<code>{backupCodes[0] ?? "ABCD-EFGH"}</code>{t("backupCodeUsage2")}
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-xs text-muted-foreground">
                          <KeyRoundIcon data-icon="inline-start" /> {summarizeCredentialId(item.credentialId)}{t("lastUpdated", { updatedAt: formatDateTime(item.updatedAt, t) })}
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
