              ...
              {pagination.pagedItems.map((row) => (
                <article key={row.id} className="rounded-lg border p-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                    <span className="rounded-md border px-2 py-1">{t("statusPrefix")}{row.status}</span>
                    <span className="rounded-md border px-2 py-1">{t("adminPrefix")}{row.isAdmin ? t("yes") : t("no")}</span>
                  </div>
                  <p className="mt-2 text-xs text-muted-foreground">
                     {t("mfaPrefix")}{row.mfa.enabled ? `TOTP:${row.mfa.totpEnabled ? t("on") : t("off")}, Passkey:${row.mfa.passkeyCount}` : t("off")}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">{t("createdAtPrefix")}{new Date(row.createdAt).toLocaleString()}</p>
                  <Button
                    variant="destructive"
                    className="mt-3 h-9 w-full"
                    onClick={() => void onDeleteUser(row.id)}
                  >
